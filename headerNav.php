<header class="burger" id="hidden">

    <nav>
        
        <div>
            <div><a href="">Profil</a></div>
            <div><i class="fas fa-user openNav"></i></div>
        </div>

        <div>
            <div><a href="">Groupes</a></div>
            <div><i class="fas fa-users openNav"></i></div>
        </div>

        <div>
            <div><a href="">Calendrier</a></div>
            <div><i class="far fa-calendar-alt openNav"></i></div>
        </div>

        <div>
            <div>
                <form>
                    <input type="text" placeholder="Chercher..">
                </form>
            </div>

            <div><i class="fas fa-search openNav"></i></div>
            
        </div>


        <div>
            <div><img src="img/brandnamewhite.png"></div>
            <div><a href="index.php"><img src="img/icon.png"></a></div>
        </div>
        
    </nav>


</header>

<script src="js/navScript.js"></script> 